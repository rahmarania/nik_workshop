...
# st.title('header')
st.write("# Empoyee Demographic Dashboard")
st.write("""Explore workforce dynamics at a glance with this Employee Demography Dashboard. Track hiring trends over time, 
         visualize employee distribution across provinces, and delve into generational and gender insights within 
         departments—all in one comprehensive view.""")

# plot part 1
col1, col2 = st.columns(2) # build 2 col

# data: line plot
col1.write("### Joining Frequency over Time") # graph title
...
col1.plotly_chart(plot_join, use_container_width = True) # adjust with col1 width


# data: map
col2.write('### Employee Count accross Indonesia')
...
# input select & slider
col3, col4 = st.columns(2) # build 2 col
input_select = col3.selectbox(label = "Select Department", options = employ_merge['department_name'].unique().sort_values(ascending = False))
input_slider = col4.slider(label = "Select Age Range", 
                           min_value = employ_merge['age'].min(), 
                           max_value = employ_merge['age'].max(), value = [20,50])

min_slider = input_slider[0]
max_slider = input_slider[1]

# plot part 2
col5, col6 = st.columns(2) # build 2 col
# data: barplot
col5.write(f"### Employee Count per Generation in {input_select} Department") # f string like glue in r
...
col6.plotly_chart(plot_dept, use_container_width = False)